...
const getFileIcon = (type: string, name: string) => {
  ...
  if (type.includes('zip') || type.includes('rar') || type.includes('7z') || 
      type.includes('tar') || type.includes('gz') || type.includes('archive')) {
  ...
  // Code files
  if (['js', 'ts', 'jsx', 'tsx', 'html', 'css', 'json', 'xml', 'md'].includes(extension || '')) {
...
export default function UploadModal({ isOpen, onClose, onNavigate }: UploadModalProps) {
  ...
  const shareViaSMS = (url: string, title: string) => {
    const text = encodeURIComponent(`Check out: ${title} - ${url}`);
    window.open(`sms:?body=${text}`);
  };

  const shareViaTwitter = (url: string, title: string) => {
  ...
  const shareViaWebAPI = async (url: string, title: string) => {
    if (navigator.share) {
      try {
            ...
            <div className="space-y-6">
              ...
              >
                ...
                <h3 className={`text-xl font-semibold mb-2 transition-colors ${
                  ...
                  {dragActive ? 'Drop files to upload' : 'Drop your files here'}
                </h3>
                <p className={`text-slate-600 mb-6 transition-colors ${
                  ...
                  {dragActive 
                    ...
                    : 'Upload multiple files of any type - images, videos, documents, and more'
                  }
                </p>
                <Input
                  type="file"
                ...
                <Button asChild variant={dragActive ? "default" : "outline"}>
                  <label htmlFor="file-upload" className="cursor-pointer">
                    {dragActive ? 'Drop Now' : 'Choose Files'}
                  </label>
                </Button>
                <p className="text-xs text-slate-500 mt-4">
                  All file types supported • No size limits • Multiple files welcome
                </p>
              </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-4 text-sm text-slate-500">
                      <span>Total: {formatFileSize(files.reduce((total, file) => total + file.size, 0))}</span>
                      {completedFiles.length > 0 && files.length > completedFiles.length && (
                  ...
                  <div className="max-h-64 overflow-y-auto space-y-2 border rounded-lg p-2">
                    {files.map((file) => (
                      <div key={file.id} className="flex items-center space-x-3 p-3 bg-white rounded-lg border shadow-sm hover:shadow-md transition-shadow">
                          ...
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              {file.status === 'completed' && (
                                <CheckCircle className="w-4 h-4 text-green-500" title="Upload completed" />
                              )}
                              {file.status === 'error' && (
                                <AlertCircle className="w-4 h-4 text-red-500" title="Upload failed" />
                              )}
                              {file.status === 'uploading' && (
                                <Loader2 className="w-4 h-4 text-blue-500 animate-spin" title="Uploading..." />
                              )}
                              <Button
                                variant="ghost"
                                size="sm"
          ...
          {step === 'complete' && generatedSite && (
            <div className="text-center py-8">
              ...
              <p className="text-slate-600 mb-8">
                Your beautiful Lytsite has been created and is ready to share with the world.
              </p>
              ...
              <Card className="max-w-lg mx-auto mb-8">
                <CardContent className="p-6">
                  ...
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Button 
                        variant="outline" 
                        size="sm"
                        onClick={() => copyToClipboard(generatedSite.url)}
                    ...
                    <div className="bg-slate-50 rounded-lg p-4 border">
                      <p className="text-sm font-mono text-slate-700 break-all select-all">{generatedSite.url}</p>
                    </div>
                  </div>

                  {/* QR Code Section */}
                  <div className="mb-6">
                    <h4 className="font-medium text-slate-900 mb-3 text-center">Quick Share</h4>
                    <div className="flex justify-center">
                      <div className="text-center">
                        <div className="w-24 h-24 bg-gradient-to-br from-slate-100 to-slate-200 rounded-xl flex items-center justify-center mb-2 border shadow-sm">
                          <QrCode className="w-12 h-12 text-slate-500" />
                        </div>
                        <p className="text-xs text-slate-500">QR Code</p>
                        <p className="text-xs text-slate-400">Scan to visit</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Share Options */}
                  <div>
                    <h4 className="font-medium text-slate-900 mb-3 text-center">Share Your Site</h4>
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      {'share' in navigator && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => shareViaWebAPI(generatedSite.url, siteTitle)}
                          className="flex items-center justify-center space-x-2 hover:bg-blue-50 hover:text-blue-700 hover:border-blue-200"
                        >
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => shareViaEmail(generatedSite.url, siteTitle)}
                        className="flex items-center justify-center space-x-2 hover:bg-green-50 hover:text-green-700 hover:border-green-200"
                      >
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => shareViaSMS(generatedSite.url, siteTitle)}
                        className="flex items-center justify-center space-x-2 hover:bg-blue-50 hover:text-blue-700 hover:border-blue-200"
                      >
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => shareViaTwitter(generatedSite.url, siteTitle)}
                        className="flex items-center justify-center space-x-2 hover:bg-sky-50 hover:text-sky-700 hover:border-sky-200"
                      >
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => shareViaFacebook(generatedSite.url)}
                        className="flex items-center justify-center space-x-2 hover:bg-blue-50 hover:text-blue-700 hover:border-blue-200"
                      >
                      ...
                      {!('share' in navigator) && (
                        <Button
                          ...
                          size="sm"
                          onClick={() => copyToClipboard(generatedSite.url)}
                          className="flex items-center justify-center space-x-2 hover:bg-slate-50 hover:text-slate-700 hover:border-slate-300"
                        >
